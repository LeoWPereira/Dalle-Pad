...
const int analogInputs = 3; // define how many pots are active up to number of available analog inputs
...
void MIDImessage(bool botao, int i, int i1, int i2);
...
void loop()
{
	...
	{ 
		...
		if ( (inputAnalog[i] / 8) != (iAlag[i] / 8) ) // if there is difference
		{
			...
			// send the MIDI
			MIDImessage(false, 176, i, ccValue[i]); // usbMIDI.sendControlChange(i, ccValue[i], 3);
   ...
   if (buttonState[i] == LOW)
     {
        ...
        MIDImessage(true, 176, cc[i], cc_on); // usbMIDI.sendControlChange(i, ccValue[i], 3);
        lastRead[i] = 1;
        }
        // set raw reading to lagged array for next comparison
      }
      ...
      if(newRead[i] != lastRead[i]){
          MIDImessage(true, 176, cc[i], cc_off); // usbMIDI.sendControlChange(i, ccValue[i], 3);
          lastRead[i] = 0;
        }
    }
   }
...
void MIDImessage(bool botao, int command, int data1, int data2) //pass values out through standard Midi Command
{
